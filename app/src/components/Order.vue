...
export default {
  ...
  methods: {
    onItemStatusChange(itemId) {
      ...
      var update = {[`bookings.${this.bookingId}.menuOrderItems.${itemId}.ready`]: true};

      db.collection('restaurants').doc(this.restaurantId).update(update).then(() => {
        const readies = Object.keys(this.booking.menuOrderItems).map(key => {
        return this.booking.menuOrderItems[key].ready
      })

        if (readies.every(ready => {return ready})) {
          var update = {[`bookings.${this.bookingId}.fulfilled`]: true}
           db.collection('restaurants').doc(this.restaurantId).update(update)
        }
      });
    }
  }
